...
**改动**：`OntologyObject` 表增加 `slug` 列（DB 层 `UniqueConstraint('tenant_id', 'slug')`）。
- 建模时 `OntologyImporter` 自动生成 slug：英文名 lowercase；中文名/含特殊字符 → 拼音或 `obj_<hash>`
- `confirm_ontology` / `edit_ontology` 维护 slug 一致性
- factory 用 slug 命名工具
- 字段同理：property 增加 slug 列；**工具参数名一律使用 `property.slug`**，description 内附原 `property.name` 供 LLM 识别业务名
- importer 在写入前先做同租户冲突检查，冲突时附 `_2` / `_3` 后缀，再落库交给 DB 约束兜底
...
- `get_specs(whitelist)` 的通配符解析由 **view 内部单点负责**：`search_*` / `count_*` 在这里做前缀匹配，不在 loader 或 executor 展开

### 2.5 每轮重新派生工具（不缓存）

自然语言建模会在 session 内频繁改 ontology（confirm/edit/rename/add-property）。**派生工具必须随 ontology 即时刷新**，否则 "刚改完字段就查询" 会用旧 schema。

**实现**：`ChatServiceV2.send_message` 每轮复用现有 `OntologyStore.get_for_project(project_id)` 取最新 ontology，再调 `factory.build(latest_ontology)` 构造新 view，喂给 ExecutorAgent。DB 查询成本是已有路径，复用现有读取，不额外引入新缓存层。Python 派生本身是字典操作，毫秒级，不缓存。
...
9. **E2E multi-project**: 同一进程内交替查询项目 A 和 B 时，tool view 能正确切换到各自的派生工具列表，不串台
