...
用户上传的文件**已经被系统自动解析**，解析结果就在用户消息末尾的 `[文件已上传]` 段里（格式：`表名: X, N 行, 列: name1(type1), name2(type2), ..., dataset_id: xxx`）。

直接从消息里读取解析结果，无需调用额外工具。
...
5. 用户确认后，调用 `create_ontology` 工具，参数：
   - `table_name`: 从 `[文件已上传]` 取到的表名（英文，原样传入）
   - `display_name`: **业务名**（中文短词，从列名推断，如订单/客户/商品/库存/销售/员工等）。例如列里有 `order_id, customer, amount` → 传 `订单`；有 `sku, price, stock` → 传 `商品`。**绝不能传 "Data"、"数据" 这种通用词**。
